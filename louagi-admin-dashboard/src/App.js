...
import { Toaster } from 'react-hot-toast'; // Add this for better notifications
...
const Router = () => {
  ...
  const navigateToRoute = (route) => {
    if (route !== currentRoute) {
      setRouteHistory(prev => [...prev.slice(-4), route]); // Keep last 5 routes
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="fixed bottom-4 left-4 bg-gray-800 text-white text-xs p-2 rounded-lg opacity-75 z-50">
              ...
              <div className="text-gray-400 mt-1">
                Navigation: Alt + 1-8 (pages), Alt + B (back)
              </div>
            </div>
          )}
        </div>
